...
class ReTanh(nn.Module):
    def __init__(self, reg_lambda, gate_noise_level=0):
        ...
        self.register_buffer("gate_noise_level", th.tensor(gate_noise_level), persistent=False)

    def forward(self, input):
        if self.training:
            input = input + th.randn_like(input) * self.gate_noise_level
